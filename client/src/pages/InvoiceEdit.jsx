...
export default function InvoiceEdit() {
  ...
  const [form, setForm] = useState({
    ...
    currency: 'INR', payment_terms: '', payment_method: '', tax_percent: 0,
    discount_type: 'flat', discount: 0, status: 'pending', reminder_enabled: true, notes: '',
  });
  ...
  useEffect(() => {
    async function load() {
      try {
        const [inv, comps] = await Promise.all([getInvoice(id), getCompanies()]);
        setForm({
          invoice_number: inv.invoice_number || '', business_name: inv.business_name || '',
          ...
          currency: inv.currency || 'INR', payment_terms: inv.payment_terms || '',
          payment_method: inv.payment_method || '', tax_percent: inv.tax_percent || 0,
          discount_type: inv.discount_type || 'flat', discount: inv.discount || 0,
          status: inv.status || 'pending', reminder_enabled: !!inv.reminder_enabled, notes: inv.notes || '',
        });
        if (inv.line_items?.length > 0) {
          setLineItems(inv.line_items.map(li => ({ description: li.description || '', quantity: li.quantity || 1, unit_price: li.unit_price || 0 })));
        }
        setCompanies(comps);
      } catch { setToast({ type: 'error', message: 'Failed to load invoice' }); }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setTimeout(() => navigate(`/invoices/${id}`), 1000);
    } catch (err) {
